...
class Collinsharper_Loomis_Model_Observer extends Varien_Object
{
    public function updateShippingBlockOptions($observer)
    {
        ...
        if ($block instanceof Mage_Checkout_Block_Onepage_Shipping_Method_Available
            ...
            $this->_addLoomisOptionCode($block, $transport, $isAdmin);
        }
    }

    protected function _addLoomisOptionCode($block, $transport, $isAdmin = false)
    {
        $change = false;
        $currentRate = false;
        $searchtagname = "s_method_loomis";
        ...
        foreach ($inputs as $input) {
            ...
            if ($isAdmin) {
                ...
                if ($valid_tag) {
                    $currentRate = str_replace('loomismodule_', '', (string) $input->getAttribute('value'));
                    ...
                    if (is_object($shippingInfoDiv)) {
                        ...
                        $input_id = 's_method_loomismodule_' . $currentRate;
    ...
    protected function _applyUpdate($doc, $block)
    {
        ...
        $script = $block->getLayout()->createBlock('core/template')
            ->setTemplate('chloomis/options_js.phtml')
            ->toHtml();
    ...
    protected function _getOptionsHtml($tag_id, $block, $visible = false)
    {
        return $block->getLayout()->createBlock('core/template')
            ->setArea('frontend')
            ->setTemplate('chloomis/options.phtml')
            ->setTagId($tag_id)
