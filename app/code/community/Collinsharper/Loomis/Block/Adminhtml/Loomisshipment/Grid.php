...
class Collinsharper_Loomis_Block_Adminhtml_Loomisshipment_Grid extends Mage_Adminhtml_Block_Widget_Grid {
    ...
    {
        parent::__construct();
        $this->setId('loomisshipment_grid');
    ...
    protected function _prepareCollection()
    {
        ...
        $collection->addFieldToFilter('o.shipping_description', array('like'=>'%loomis%'));
        $tableName = Mage::getSingleton('core/resource')->getTableName('ch_loomis_shipment');

        $collection->getSelect()->joinLeft(array('ch_shipment'=>$tableName), 'main_table.entity_id=ch_shipment.magento_shipment_id',array(
            'loomis_shipment_id'=>'shipment_id',
    ...
    protected function _prepareColumns()
    {
       ...
       $this->addColumn('shipment_id', array(
        ...
        $this->addColumn('loomis_shipment_id', array(
            'header'    => Mage::helper('sales')->__('Loomis Shipment #'),
            'index'     => 'loomis_shipment_id',
            'filter_index'     => 'ch_shipment.shipment_id',
            'type'      => 'text',
        ));

        $this->addColumn('manifest_id', array(
            'header'    => Mage::helper('sales')->__('Loomis Manifest #'),
            'index'     => 'manifest_id',
            'renderer'  => 'loomismodule/adminhtml_loomisshipment_renderer_manifestId',
    ...
    protected function _prepareMassaction()
    {
        ...
        $this->getMassactionBlock()->addItem('create_shipment', array(
            'label'=> Mage::helper('sales')->__('Create Loomis Shipments'),
            'url'  => $this->getUrl('*/loomisshipment/massCreate'),
        ));

        $this->getMassactionBlock()->addItem('void_shipment', array(
            'label'=> Mage::helper('sales')->__('Void Loomis Shipments'),
            'url'  => $this->getUrl('*/loomisshipment/massVoid'),
        ));

        $this->getMassactionBlock()->addItem('get_label', array(
            'label'=> Mage::helper('sales')->__('Print Labels'),
            'url'  => $this->getUrl('*/loomisshipment/massLabels'),
        ));

        $this->getMassactionBlock()->addItem('get_tlabel', array(
            'label'=> Mage::helper('sales')->__('Print Thermal Labels'),
            'url'  => $this->getUrl('*/loomisshipment/massLabels', array('_thermal' => true)),
        ));
